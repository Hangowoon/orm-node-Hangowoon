...
function fnPlaus1(a,b, callBack){
    let c=a+b;
    callBack(c);
    return c;
}
...
//로직을 변경/추가하여 변경된값을 출력한다.
function logging1(result){
    var total = 3000+result;
    console.log(`계산결과는 배송비가 추가되어 ${total} 원입니다.`);
}

var result1 = fnPlaus1(20,30,logging);
var result2 = fnPlaus1(20,30,logging1);
var result3 = fnPlaus1(20,30,function(result){
    var total = 3000+result;
...
//전달된 콜백함수를 실행시켜 원하는 로직/절차를 순차적으로 구현되게할 수 있다. 
...
//추상화, 상속, 다양성, 캡슐화
//추상화(asbstraction) : 상속(부모클래스를 상속받아 자식클래스를 만드는것)과 
//추상화 인터페이스(미리 클래스에서 구현해야하는 기능과 속성 정의만 한다. - 구현은 하지 않는다.)